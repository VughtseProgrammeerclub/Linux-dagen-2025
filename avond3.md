# Linux dagen avond 3: Terminal introductie

## Introductie
We beginnen met een kleine demo en toelichting van de command line. Daarna kan ieder in zijn eigen tempo aan de volgende tutorials werken. Let op: de tutorials hebben ook een mogelijkheid om in een online omgeving te oefenen, maar met een gratis account kun je maar 3 van de 40 lessen op 1 dag doen, dus voer de commando's in je eigen shell uit.

* [Tutorial 1: Linux Journey: The Shell](https://labex.io/lesson/the-shell)
* [Tutorial 2: Linux Journey: Text-Fu](https://labex.io/lesson/stdout-standard-out-redirect)

Voor de shell kun je ook ingewikkeldere programma's schrijven in shell scripts. Net echt programmeren met variabele en for loops. In dit soort kleine scriptjes blinken AI tools uit, dat kunnen ze heel goed voor je genereren. Maar voor wie er zelf mee aan de gang gaat (of wil snappen wat AI allemaal bedenkt) is hier de [learn bash scripting in 10 minutes](https://dev.to/arindam_1729/learn-bash-scripting-in-10-minutes-1jec) tutorial.

## Voor gevorderden
Voor de iets gevorderde shell gebruiker is er een Security challenge puzzel, waarin je met 6 shell commando's in elk level het wachtwoord moet achterhalen.

* [OverTheWire WarGames Bandit](https://overthewire.org/wargames/bandit/bandit0.html)
* [Uitgewerkte uitleg (SPOILERS!)](https://mayadevbe.me/posts/overthewire/bandit/level1/)

## Leesvoer
Er zijn ook veel boeken over werken met de shell en het scripten er in geschreven. Hier zijn een paar gratis boeken uit de [Free-DevOps-Books-1 repository](https://github.com/rootusercop/Free-DevOps-Books-1/):

* Learning Linux Shell Scripting [[Download]](/book/Learning%20Linux%20Shell%20Scripting.pdf)
* The Linux Command Line - A Complete Introduction [[Download]](/book/The%20Linux%20Command%20Line%20-%20A%20Complete%20Introduction.pdf)
* Linux Shell Scripting Essentials [[Download]](/book/Linux%20Shell%20Scripting%20Essentials.pdf)
* Linux Shell Scripting Cookbook [[Download]](/book/Linux%20Shell%20Scripting%20Cookbook.pdf)
* Advanced Bash-Scripting Guide [[Download]](/book/Advanced%20Bash-Scripting%20Guide.pdf)
* Beginning the Linux Command Line [[Download]](/book/Beginning%20the%20Linux%20Command%20Line.pdf)
